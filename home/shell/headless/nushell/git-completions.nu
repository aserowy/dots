...
def "nu-complete git checkout targets" [] {
  let locals = (git branch
    | lines
    | each { |line| $line | str replace '\* ' "" | str trim }
    | each { |line| { value: $line, description: "local" }}
  )
  ...
  let remotes = (git remote
    ...
    | flatten
    | each { |line| { value: $line, description: "remote" }}
    #| each { |line| $"(ansi red)($line)(ansi default)" }
  )
...
export extern "git checkout" [
  ...targets: string@"nu-complete git checkout targets"   # name of the branch or files to checkout
